...
<%
				...
				try {
					...
					try{
						...
						String sql = "select start_date, end_date from score_date";
					pstmt = conn.prepareStatement(sql);
					rs = pstmt.executeQuery();
					rs.next();
					...
					if(today.getTime() < s_date.getTime() || today.getTime() > e_date.getTime()) {
						%><script>alert("성적확인 기간이 아닙니다."); history.back();</script><%
					}
					else {
						String sql2 = "select * from score where student_id=?";
...
<%						
						...
						while(rs2.next()) {
							if(count%2==0) {
						%>	<tr>
								<td class="even"><%=rs2.getString("subject_number")%></td>
								<td class="even"><%=rs2.getString("subject")%></td>
								<td class="even"><%=rs2.getString("score")%></td>
							</tr>	
			<%				}
							else {
					%>			<tr>
								<td><%=rs2.getString("subject_number")%></td>
								<td><%=rs2.getString("subject")%></td>
								<td><%=rs2.getString("score")%></td>
							</tr>	
			<%				}
							count++;
						}

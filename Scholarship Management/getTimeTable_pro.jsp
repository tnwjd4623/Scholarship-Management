...
<%			
				try {
					...
					try{
						...
						String sql3 = "select name, classroom, time, number, day from subject where professor=?";
					...
					%><br/><br/>강의 시간표:  <br/><br/>
						...
						while(rs3.next()) {						
							if(count%2 == 0) {
								%> <tr>
										<td class="even"><%=rs3.getString("number") %></td>
										<td class="even"><%=rs3.getString("name") %></td>
										<td class="even"><%=rs.getString("name")%></td>
										<td class="even"><%=rs3.getString("classroom")%>
										<td class="even"><%=rs3.getString("time")%>
										<td class="even"><%=rs3.getString("day")%>
								   </tr>
								<%
							}
							else {
								%> <tr>
								<td><%=rs3.getString("number") %></td>
								<td><%=rs3.getString("name") %></td>
								<td><%=rs.getString("name")%></td>
								<td><%=rs3.getString("classroom")%>
								<td><%=rs3.getString("time")%>
								<td><%=rs3.getString("day")%>
						   </tr>
						<%
							}
							count++;
